...
# mkt-campaign-guardrail`, `adapters/gcp/model_armor_guardrail.py` posts every screen to
# `.../locations/<region>/templates/mkt-campaign-guardrail:sanitize*`, `iam.tf` grants
# `roles/modelarmor.user` and `apis.tf` enables the API, and nothing created the template. Every
# input and output screen on a deployment would have been refused by a template that was never
# provisioned. `marketing-compliance-gate` had the same defect; this reuses its recorded fix
# (`marketing-compliance-gate/infra/terraform/model_armor.tf:3-6`).
#
# Control map (SPEC concern):
#   Guardrail screening: prompt injection, jailbreak and the responsible-AI categories on both
#     directions of every plan.
...
resource "google_model_armor_template" "mkt_campaign_guardrail" {
  provider    = google-beta
  project     = var.project_id
  location    = var.region               # the regional endpoint the adapter calls
  template_id = "mkt-campaign-guardrail" # matches settings.yaml model_armor.template_id
