...
variable "model_armor_full_capabilities" {
  ...
  description = <<-EOT
    ...
    not to. asia-southeast1 (this stack's default region, allowed_regions) serves neither, and
    Model Armor does not degrade: it refuses the template with CAPABILITY_NOT_SUPPORTED, so the
    stack does not deploy at all. A deployment there sets this false, which narrows the guardrail
    and is a disclosure to make in the deployment's posture record rather than a silent downgrade.
  EOT
}
...
variable "guardrail_enabled" {
  description = "Switch the input and output guardrail (Model Armor under gcp) on the service (MKT_CAMPAIGN_GUARDRAIL). A cheap runtime control: on in the reference, reversible, so it takes a default."
  type        = bool
  default     = true
}

variable "review_routing_enabled" {
  description = "Switch the hand-off of every plan to the human-review-console (MKT_CAMPAIGN_REVIEW_ROUTING). A cheap runtime control: on in the reference, reversible, so it takes a default."
  type        = bool
  default     = true
}
...
variable "human_review_url" {
  description = <<-EOT
    The human-review-console base URL the review router submits every plan to
    ...
    then may pass ""). HTTPS, because the payload carries the plan.
  EOT
  type        = string
