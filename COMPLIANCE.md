# COMPLIANCE: Mkt2 Campaign Planning and Budget-Allocation Assistant

This maps every General Principle (P-01..P-13) and dependency rule (R1..R8) to a concrete
control in **this** repo. Where a principle does not apply to Mkt2, it is marked **n/a** with
the reason. Mkt2 plans over aggregate audience benchmarks and no per-customer PII, so its
...
| # | Principle | How Mkt2 implements it | Evidence |
|---|-----------|----------------------|----------|
...
| **P-04** | Minimise data to the model | Mkt2 sends aggregate audience benchmarks and no per-customer PII; the model-boundary callback still guardrail-screens every prompt and response, and spans capture no content | `agent/callbacks.py`, `domain/plan_service.py` |
...
| **P-08** | Eval-gated promotion | Offline eval gate scores plan quality (allocation sanity, brief groundedness, review safety); Hrz4 at promotion | `eval/run_eval.py`, `ports/observability.py` (`EvaluationGatePort.gate`) |
...
| **P-13** | Fair, consented marketing (advertising compliance) | Mkt2 produces an internal plan (audience, allocation, brief), not published advertising; any output that becomes customer-facing must pass Mkt6 (rule R7). The agent instruction forbids drafting final customer-facing ad copy here | `agent/root_agent.py` instruction, R7 below |

---

## Dependency rules

Mkt2's mandatory dependencies are **Hrz1, Hrz2, Hrz3, Hrz4 (gate), Hrz5 and Mkt1** (see
...
| Rule | Requirement | How Mkt2 satisfies it | Evidence |
|------|-------------|---------------------|----------|
| **R1** | Customer PII handling: Hrz1 guardrail + DLP redaction | Mkt2 consumes the Hrz1 guardrail for prompt-injection and unsafe-output screening (INPUT and OUTPUT, pipeline and model boundary). **PII redaction is n/a**: Mkt2 plans over aggregate audience benchmarks with no per-customer record (C2/C3/C4 n/a in the practices audit) | `ports/safety.py`, `domain/plan_service.py`, `agent/callbacks.py` |
| **R2** | Audit to Hrz5 | Every plan writes an immutable WORM `AuditEvent`; the `platform` adapter posts to Hrz5 `/v1/audit` | `adapters/gcp/cloud_logging_audit.py`, `adapters/platform/remote_audit.py` |
| **R3** | Governed RAG via Hrz2 | **n/a in-repo**: Mkt2 has no RAG retrieval step; audience benchmarks come from BigQuery. If a brand-corpus grounding step is added it routes through Hrz2 | audience data via `ports/audience.py` |
...
| **R6** | Validated by Rsk3 at intake | As a new project, Mkt2 is validated by the Rsk3 intake validator externally. n/a in-repo | intake handled by Rsk3 externally |
| **R7** | Marketing compliance via Mkt6 | Mkt2 produces an internal plan, not published advertising. Any output that becomes customer-facing must pass Mkt6 (per-market advertising / consumer-protection claim check, brand guidelines, marketing consent) and screen via Hrz1. The agent forbids drafting final customer-facing copy here | `agent/root_agent.py` instruction; Mkt6 governance |
| **R8** | Route escalations to Hrz7 | A `Plan` sets `requires_human_review`, so it MUST be routed to the Hrz7 Human-Review & Maker-Checker Console via the shared `review-kit`, not terminate in a per-repo boolean. The producer redacts the descriptor, summary and citation snippets before the wire (defense in depth), threads the verified actor as maker and the tenant from the call, and defaults severity to `medium` (a `Plan` carries no severity signal). `local` enqueues to an in-memory outbox; `platform`/`gcp` submit over S2S; `onprem` is the fail-fast sovereign placeholder | `ports/review_router.py`, `adapters/_review_payload.py`, `adapters/local/review_router.py`, `adapters/platform/review_router.py`, `adapters/onprem/review_router.py` |

---

## Why Mkt2 has no per-customer PII surface (R1, C2..C4)
...
- **Aggregate inputs only.** A plan is built from aggregate audience segments and per-channel
  ...
  customer identifier and no tenant-partitioned customer data (contrast Mkt5, the
...
| Mkt2 control | Reference regime | What a supervisor looks for |
|---|---|---|
...
counsel. The Mkt2-control column is stable across regulators; only the mapping changes. The
