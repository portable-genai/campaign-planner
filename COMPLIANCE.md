...
| **P-09** | Defense in depth / zero trust | CMEK (named on the audience dataset AND again on every table in it, because it cascades in BigQuery and not in Terraform's model, where an undeclared block reads as a key removal and replaces the table), least-privilege IAM, private endpoints, a distinct agent identity; the guardrail screens twice (domain pipeline and model-boundary callback) | `infra/terraform/kms.tf`, `bigquery.tf`, `iam.tf`, `agent/callbacks.py` |
