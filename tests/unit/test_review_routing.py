"""R8 routing: an escalated campaign plan is routed to human-review-console via the shared
review-kit.

Every campaign plan is spend-affecting and always requires human review, so rule R8 says it MUST be
handed to the human-review-console maker-checker console rather than left as a per-repo boolean.
These tests prove the producer half of that loop end-to-end against the offline local router (an
in-memory outbox), and prove the redact-before-wire boundary so no stray contact identifier reaches
the console. All data here is fictional.
"""
...
def test_build_plan_routes_escalated_plan_to_outbox(
    ...
    assert len(pending) == 1, (
        "the escalated plan must be routed to human-review-console exactly once"
    )
