...
The defect this locks out is absence read as consent: ``MKT_CAMPAIGN_PROFILE`` unset used to
...
from campaign_planner.adapters.local.identity import (
...
_SRC = Path(__file__).resolve().parents[2] / "src" / "campaign_planner"
...
def test_an_EMPTIED_variable_refuses_rather_than_inheriting_the_unset_default() -> None:
    """The docstring here must never read "unset and set-but-empty are the same state".
...
def test_an_unconsented_run_gets_no_cross_origin_trust() -> None:
    ...
    choice = resolve_profile({})
    assert cors_allowlist(choice.exposure_profile, origins_env="MKT_CAMPAIGN_CORS_ORIGINS") == []
    assert cors_allowlist("local", origins_env="MKT_CAMPAIGN_CORS_ORIGINS") != []
