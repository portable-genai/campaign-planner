...
needs; and every caller that hands a plan to the review router says what happened to it.
...
from campaign_planner.adapters.controls import (
    DisabledGuardrail,
...
from campaign_planner.config import (
    GUARDRAIL_ENV,
...
from campaign_planner.envread import ConfiguredEmptyError

_PROFILE_ENV = "MKT_CAMPAIGN_PROFILE"
...
def test_a_process_with_a_control_off_says_so_at_startup(
    ...
    with caplog.at_level(logging.WARNING, logger="campaign_planner.config"):
...
# --------------------------------------------------------------------------- #
class _Accepting:
    def route(self, plan: object, *, maker: str, tenant: str = "") -> None:
        return None
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="campaign_planner.adapters.controls"):
        failed.route(object(), maker="m")  # type: ignore[arg-type]
    assert failed.outcome is ReviewRouting.FAILED
...
# --------------------------------------------------------------------------- #
# Every caller reports what happened to the plan it handed off
# --------------------------------------------------------------------------- #
...
def _post_plan(monkeypatch: pytest.MonkeyPatch, container: Container) -> dict[str, Any]:
    monkeypatch.setattr(deps, "get_container", lambda: container)
    response = TestClient(app, client=LOOPBACK_PEER).post("/v1/plan", json=_PLAN_BODY)
    assert response.status_code == 200, response.text
    body: dict[str, Any] = response.json()
    return body


def test_the_api_reports_a_routed_plan(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_plan(monkeypatch, _container(local_settings))
...
def test_the_api_reports_routing_off(
    ...
    body = _post_plan(monkeypatch, _container(local_settings, review_routing=False))
    assert body["review_routing"] == "off"


def test_the_api_reports_a_failed_hand_off_instead_of_failing_the_plan(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_plan(monkeypatch, _container(local_settings, router=_Refusing()))
...
def test_the_agent_tool_reports_the_hand_off(local_settings: Settings) -> None:
    ...
    payload = tools.build_campaign_plan("savings account acquisition", 120000.0, settings=settings)
    assert payload["review_routing"] == "off"


def test_the_mcp_plan_tool_reports_the_hand_off(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from campaign_planner.mcp import server as mcp_server

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, router=_Refusing())
    )
    payload = mcp_server.build_handlers(actor="svc:test")["build_plan"](
        objective="savings account acquisition", market="SG", vertical="banking", total_budget=1e5
    )
    assert payload["review_routing"] == "failed"


def test_the_cli_says_where_the_plan_went(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from campaign_planner.cli.main import app as cli

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, review_routing=False)
    )
    result = CliRunner().invoke(cli, ["savings account acquisition"])
