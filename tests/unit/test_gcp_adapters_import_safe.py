...
from campaign_planner.domain.errors import UnsupportedMarketError
from campaign_planner.domain.models import Market
...
GCP_ADAPTERS: dict[str, tuple[str, type]] = {
    "audience": (
        "campaign_planner.adapters.gcp.bigquery_audience:BigQueryAudienceDataAdapter",
        ports.AudienceDataPort,
    ),
    "llm": (
        "campaign_planner.adapters.gcp.gemini_llm:GeminiLLMAdapter",
        ports.LlmPort,
    ),
    "guardrail": (
        "campaign_planner.adapters.gcp.model_armor_guardrail:ModelArmorGuardrailAdapter",
        ports.GuardrailPort,
    ),
    "audit": (
        "campaign_planner.adapters.gcp.cloud_logging_audit:CloudLoggingAuditAdapter",
        ports.AuditSinkPort,
    ),
    "tracer": (
    ...
    "evaluation": (
        "campaign_planner.adapters.gcp.genai_eval:GenAiEvalAdapter",
        ports.EvaluationGatePort,
    ),
    "agent_registry": (
        "campaign_planner.adapters.gcp.a2a_registry:A2ARegistryAdapter",
        ports.AgentRegistryPort,
    ),
    "tool_catalog": (
        "campaign_planner.adapters.gcp.mcp_tool_catalog:McpToolCatalogAdapter",
...
def test_gcp_adapter_constructs_and_satisfies_protocol(port_name: str):
    ...
    from campaign_planner.config import instantiate
...
def test_region_validation_accepts_each_apac_market():
    from campaign_planner.adapters.gcp._region import resolve_region
