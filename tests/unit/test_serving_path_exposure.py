...
Red before the fix, proved by execution: the planner had no app-bound exposure guard at all, and
the shipped entry points both hand the app object straight to uvicorn:

    Dockerfile:  CMD exec uvicorn campaign_planner.api.app:app --host 0.0.0.0 --port ${PORT}
    Makefile:    run-api -> $(BIN)/uvicorn campaign_planner.api.app:app --host $(API_HOST) ...
...
from campaign_planner.adapters.gcp.iap_identity import IapIdentityAdapter
from campaign_planner.adapters.local.identity import LocalPersonaIdentityAdapter
from campaign_planner.adapters.onprem.identity import OnPremIdentityAdapter
from campaign_planner.api import app as app_module
from campaign_planner.api import deps
from campaign_planner.config import Settings, end_user_auth_kind, identity_adapter_class
from campaign_planner.ports.identity import (
    CLIENT_ASSERTED,
    END_USER_AUTH_ATTR,
    END_USER_AUTH_KINDS,
    UNIMPLEMENTED,
...
_APP_SOURCE = _REPO_ROOT / "src" / "campaign_planner" / "api" / "app.py"
...
def test_a_lan_peer_is_refused_the_consequential_route_too() -> None:
    """The persona list was the way in; the spend-affecting route was the thing worth reaching."""
    response = _client(LAN_PEER).post(
        "/v1/plan",
        json={
...
def test_the_shipped_entry_points_serve_the_app_object() -> None:
    ...
    assert "campaign_planner.api.app:app" in dockerfile
    assert "--host 0.0.0.0" in dockerfile
    assert "campaign_planner.api.app:app" in makefile
...
def _settings(profile: str) -> Settings:
    ...
    return Settings(profile=profile, adapters=base.adapters)
...
def test_the_posture_follows_a_REBOUND_identity_port_not_the_profile_name() -> None:
    ...
    rebound["identity"]["onprem"] = "campaign_planner.adapters.gcp.iap_identity:IapIdentityAdapter"
...
def test_an_unresolvable_binding_fails_CLOSED_rather_than_raising_past_the_guard() -> None:
    ...
    broken["identity"]["local"] = "campaign_planner.nope:Missing"
...
_MUTANT = (
    "_TOKEN_ENV = 'MKT_CAMPAIGN_S2S_TOKEN'\n"
    "_ZERO_SECRET_LOCAL = (\n"
