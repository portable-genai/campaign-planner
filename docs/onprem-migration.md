...
The whole point of the ports-and-adapters shape is that `campaign-planner`'s exit story is **demonstrable,
...
To run `campaign-planner` on a sovereign / on-premise platform, implement these adapter bodies (the only
files that change):
