...
:class:`~campaign_planner.domain.identity.RequestContext` (the request headers) and gets back a
verified :class:`~campaign_planner.domain.identity.Principal`, or an
:class:`~campaign_planner.domain.identity.IdentityError`. The active profile picks the adapter:
...
other port, and is the single seam where a client-asserted actor would be replaced by a
server-verified one.

The Protocol itself is NOT declared here: it is re-exported from :mod:`hex_service_kit.identity`,
so this repo's adapters bind the same object every other repo binds. See
``ports/observability.py`` for why a hand-copied Protocol is a defect rather than a convenience.
"""
