...
(``HRZ_REVIEW_URL``) and the S2S bearer/actor credentials reuse this repo's platform S2S env
convention (:mod:`campaign_planner.adapters.platform._s2s`: ``HRZ_S2S_TOKEN`` /
``HRZ_S2S_SIGNING_KEY``), so the review console is authenticated the same way as every other
...
class PlatformReviewRouter:
    ...
    def route(  # pragma: no cover - needs live Hrz7
        ...
        base_url = read_env_setting("HRZ_REVIEW_URL").value
        if not base_url:
            raise RuntimeError("HRZ_REVIEW_URL must be set to route reviews to Hrz7")
        client = ReviewClient(
            base_url,
