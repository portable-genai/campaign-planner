...
(``MKT_CAMPAIGN_GUARDRAIL``, ``MKT_CAMPAIGN_REVIEW_ROUTING``), the container binds one of these
...
**Recording wrapper.** Every caller that hands a plan to the router (the API route, the agent
tool, the CLI, the MCP ``build_plan`` tool) wraps the bound router in
...
because an already-audited plan must not fail on a console outage, but it is no longer
...
from ..domain.models import Direction, GuardrailVerdict, Plan
...
class RecordingReviewRouter:
    ...
    def route(self, plan: Plan, *, maker: str, tenant: str = "") -> None:
        if not getattr(self._inner, "enabled", True):
            ...
            return
        try:
            self._inner.route(plan, maker=maker, tenant=tenant)
