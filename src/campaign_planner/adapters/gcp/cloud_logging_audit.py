"""Cloud Logging WORM audit adapter (AuditSinkPort) — GCP managed stack for D2.
...
routes the ``campaign-planner-audit`` log to a **locked log bucket** (WORM, ~7-year
retention), so records are write-once and immutable: the regulator-grade audit guarantee
(A5) for every spend-affecting plan.
