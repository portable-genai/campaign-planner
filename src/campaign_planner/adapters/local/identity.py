...
These personas authenticate NOBODY, so this adapter refuses to construct unless the local
profile was chosen deliberately: the profile must actually be ``local`` AND (when the
settings came from the environment) ``MKT_CAMPAIGN_PROFILE`` or the settings file must have
named it, rather than it being inherited from a fallback. A missing variable therefore fails
closed instead of serving a spend-planning API to any caller as a seeded analyst.

The persona ids, subjects and tenants are the catalog-wide seeded demo identities (shared
with the sibling repos) so a cross-repo demo shows one consistent cast of users.
"""
...
_PERSONAS: tuple[Principal, ...] = (
    Principal(
        ...
        principals=("group:mkt-planner", "group:marketing"),
        tenant="demo-bank",
    ...
    Principal(
        ...
        principals=("group:mkt-planner", "group:marketing", "group:mkt-approver"),
        tenant="demo-bank",
        assurance="local-demo",
...
class LocalPersonaIdentityAdapter:
    ...
    #: The persona arrives on the ``X-Dev-Persona`` header the CALLER wrote. That is a
    #: selection, not a credential, so this adapter authenticates nobody and may never buy
    #: the exposure guard's relaxation.
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "MKT_CAMPAIGN_PROFILE is not set, so the local profile was inherited rather "
                "than chosen; the seeded dev personas authenticate nobody and are refused. "
                "Set MKT_CAMPAIGN_PROFILE=local deliberately for a dev or demo run, or "
                "MKT_CAMPAIGN_PROFILE=gcp for a real deployment."
            )
