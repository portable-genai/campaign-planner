...
Exercises the R8 routing path offline: an escalated plan is converted to a review and enqueued (the
...
class LocalReviewRouter:
    ...
    def route(self, plan: Plan, *, maker: str, tenant: str = "") -> None:
        self._outbox.enqueue(
            plan_to_review(plan, maker=maker, tenant=tenant), actor="campaign-planner"
        )
