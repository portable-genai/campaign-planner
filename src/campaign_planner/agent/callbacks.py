...
The domain service already screens INPUT and audits inside its pipeline (SPEC §5). These ADK
callbacks add a **second, independent line of defence at the model boundary**: every prompt
...
D2 plans campaigns over aggregate audience benchmarks and no customer PII (C2/C3/C4 n/a in the
practices audit), so there is no PII-redaction callback here. The guardrail's own
``sanitized_text`` is still honoured, so any content the policy chooses to mask never reaches
the model or the audit record.

The callbacks are built from a :class:`~campaign_planner.config.Container`, so the active
...
who has already pinned it). ADK imports are done lazily inside the factory / callbacks so this
