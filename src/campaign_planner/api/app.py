...
Identity is server-verified: every artifact route resolves a :class:`Principal` through the
active profile's IdentityPort (``api/security.py``) and uses it as the audit actor. The API
never accepts a client-supplied ``actor``. Embedding-surface controls (env-driven CORS
allowlist + CSP ``frame-ancestors``) let the UI run standalone or embed inside a client
portal (see ``docs/embedding-and-identity.md``).
"""
...
from ..domain.models import Channel, Market, PacingStrategy, PlanRequest, Vertical
from ..domain.serialization import to_jsonable
from ..ports.identity import VERIFIED
from . import deps
from .deps import make_plan_service
from .schemas import AgentCardModel, HealthModel, PlanRequestModel
...
# Embedding-surface controls. In secure/embedded mode the planner is served same-origin via
# the parent app's reverse-proxy (no CORS needed); for the cross-origin / standalone dev
# case, MKT_CAMPAIGN_CORS_ORIGINS is an explicit per-tenant allowlist (never "*").
# MKT_CAMPAIGN_FRAME_ANCESTORS is the CSP frame-ancestors allowlist of parent origins
# permitted to iframe the planner UI.
_FRAME_ANCESTORS_ENV = "MKT_CAMPAIGN_FRAME_ANCESTORS"
_CORS_ORIGINS_ENV = "MKT_CAMPAIGN_CORS_ORIGINS"
...
_INSECURE_DEMO_ENV = "MKT_CAMPAIGN_ALLOW_INSECURE_DEMO"
    ...
    * set with a value: used as given, once :func:`_refuse_wildcard` has established that it
    ...
    if setting.is_configured_empty:
        raise ConfiguredEmptyError(
            ...
            "is discarded by browsers, which would leave the planner with no clickjacking "
...
app = FastAPI(
    title="D2 Campaign Planning and Budget Allocation",
    version="0.1.0",
    description=(
        "Cited campaign plans (audience selection, channel-mix budget allocation, reach / "
        "frequency, pacing) generic across banking and online retail and the JP/AU/SG "
        "markets, with maker-checker review on every spend-affecting plan."
    ),
)
...
# showing the configured `generator_model` from `/healthz`. It also sends
# `Access-Control-Expose-Headers` naming both, which is what lets this console, calling the
# service cross-origin, read them at all.
install_answer_provenance(app)


@app.middleware("http")
async def _security_headers(request: Request, call_next: Any) -> Any:
    """Emit embedding-surface headers: CSP frame-ancestors (who may iframe the planner).
...
#
#     CMD exec uvicorn campaign_planner.api.app:app --host 0.0.0.0 --port ${PORT}
#
# and ``make run-api`` hands the same ``campaign_planner.api.app:app`` object to uvicorn. Both
...
# full seeded-persona list and could then act as any of them by echoing the id back in
# ``X-Dev-Persona``.
...
def personas() -> list[dict[str, str]]:
    ...
    via the ``X-Dev-Persona`` header. Secure profiles resolve identity from the IAP
    assertion, so this returns an empty list, and so does a run that never chose a profile:
    the persona adapter refuses to construct there, which is an empty picker rather than a
    500.
    """
    try:
...
@app.post("/v1/plan")
def build_plan(body: PlanRequestModel, principal: CurrentPrincipal) -> dict:
    try:
        request = PlanRequest(
            ...
            flight_legs=body.flight_legs,
            pacing=PacingStrategy(body.pacing),
            max_segments=body.max_segments,
            channels=tuple(Channel(c) for c in body.channels),
            effective_frequency=body.effective_frequency,
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    # The hand-off never fails an already-built, already-audited plan; the response says what
    ...
    try:
        plan = make_plan_service(review_router=routing).build_plan(
