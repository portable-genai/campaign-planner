# D2 Campaign Planning and Budget Allocation — developer tasks.
#
...
export MKT_CAMPAIGN_PROFILE := $(PROFILE)
...
ui-check: ## The console gate: types, policy unit tests, build, then hydration against the BUILT server.
...
demo: ## Offline demo: run the plan flow + render the static audit-first HTML (scripts/out).
	MKT_CAMPAIGN_PROFILE=local PYTHONPATH=src $(BIN)/python scripts/demo.py
	MKT_CAMPAIGN_PROFILE=local PYTHONPATH=src $(BIN)/python scripts/render_plan_ui.py scripts/out

demo-server: ## Live, presenter-controlled offline demo server on :$(DEMO_PORT).
	MKT_CAMPAIGN_PROFILE=local PYTHONPATH=src $(BIN)/python scripts/demo_server.py --port $(DEMO_PORT)

demo-selftest:
	MKT_CAMPAIGN_PROFILE=local PYTHONPATH=src $(BIN)/python scripts/demo_selftest.py

demo-browser: ## Drive the SERVED presenter demo through a real headless browser ([demo] extra).
	MKT_CAMPAIGN_PROFILE=local $(BIN)/pytest tests/browser -q -rs

smoke-local: ## End-to-end offline smoke: build a cited plan under the local profile.
	MKT_CAMPAIGN_PROFILE=local $(BIN)/mkt-campaign "savings account acquisition" -m SG -v banking -b 120000
