...
const HERE = "http://console.test/plan";
const STANDALONE = "http://api.test:8101";
const PORTAL = "/planner/api";
...
test("only this console's own API base counts, standalone or under the portal", () => {
  assert.equal(isConsoleApi(STANDALONE + "/v1/plan", HERE, STANDALONE), true);
  assert.equal(isConsoleApi(new URL(STANDALONE + "/healthz"), HERE, STANDALONE + "/"), true);
  assert.equal(isConsoleApi({ url: STANDALONE + "/v1/plan" }, HERE, STANDALONE), true);
  assert.equal(isConsoleApi("http://elsewhere.test:8101/v1/plan", HERE, STANDALONE), false);
  assert.equal(isConsoleApi("/planner/api/v1/plan", HERE, PORTAL), true);
  assert.equal(isConsoleApi("/planner/apisomething", HERE, PORTAL), false);
...
test("the wrapper reports an answer from the console's API and ignores other origins", async () => {
  ...
  await window.fetch(STANDALONE + "/v1/plan", { method: "POST" });
  await window.fetch("http://elsewhere.test/v1/plan");
  stop();
...
test("the wrapper is installed once however many listeners, and restored by the last", async () => {
  ...
  await window.fetch("/planner/api/v1/plan");
  stopFirst();
