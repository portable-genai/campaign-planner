...
// CSP string, and the string was right. `tsc --noEmit` was clean. `next build` succeeded. The page
// rendered, the headers were correct, and a screenshot looked exactly like a working console.
// What was actually shipped was dead markup: a `script-src` with no nonce blocks Next's inline
...
//
//   1. Every directive the policy promises is present, and none of them is empty. A directive with
//      no value is a parse error browsers DISCARD, so an empty `frame-ancestors` removes the
//      restriction it was written to apply.
//   2. The response carries a nonce in `script-src`.
//   3. EVERY `<script>` tag in the document carries that same nonce.
//
// Rule 3 is the one that matters, and it is the one a header assertion cannot express: the header
// is byte-identical in the working and the broken case. A statically prerendered page was built
// before the nonce existed, so it emits script tags with no nonce while the header advertises one,
// and because `'strict-dynamic'` disables the `'self'` fallback, that combination blocks strictly
// MORE than the unfixed policy did. Header and markup have to agree, and only the markup knows.
//
...
const BOOT_TIMEOUT_MS = 90_000; // a cold `next start` on CI is slower than it looks
const POLL_MS = 250;

/** Directives the console's policy must always carry. Absence is the fleet defect, not a bad value. */
