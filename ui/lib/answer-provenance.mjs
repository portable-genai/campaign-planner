...
// response headers the console reads. A request that noted nothing carries neither, so nothing
// here ever guesses a model: no header, no answer.
//
// This console calls its service directly at `API_BASE` (the service's own origin standalone,
// `<mount>/api` under the portal). A cross-origin response hides every header it does not
// expose, so the kit middleware also sends `Access-Control-Expose-Headers` naming both.
//
...
/**
 * True when `input` addresses this console's own API: the origin and path of `base`.
 *
 * @param {unknown} input what was handed to `fetch`
 * @param {string} here the page's own URL
 * @param {string} base the console's API base (absolute, or a path on this origin)
 */
export function isConsoleApi(input, here, base) {
  ...
  try {
    const root = new URL(base.replace(/\/+$/, "") + "/", here);
