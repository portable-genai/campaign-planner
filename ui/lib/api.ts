...
/**
 * Typed fetch client for the D2 Campaign Planner FastAPI backend.
 *
 * Routes:
 *   POST /v1/plan      -> Plan
 *   GET  /healthz      -> Health
 ...
 *
 * The API base is read from NEXT_PUBLIC_API_BASE (default http://localhost:8101, the D2
 * API port). All calls are thin: the backend owns the business logic and the citations.
 * The backend also owns identity: it never accepts a client-supplied actor. In LOCAL mode
 * it resolves a seeded dev persona from the X-Dev-Persona header; in secure profiles that
 * header is ignored (identity comes from the IAP-verified assertion).
 */

import type { Health, Market, Pacing, Persona, Plan, Vertical } from "./types";
...
const DEFAULT_API_BASE = "http://localhost:8101";
...
// Dev-only identity selection (the persona picker). Attached as X-Dev-Persona only when
// set; the secure profiles resolve identity server-side and ignore it.
let devPersona = "";
...
export async function buildPlan(body: PlanBody, signal?: AbortSignal): Promise<Plan> {
  ...
  return (await parseJsonOrThrow(res)) as Plan;
}
...
export const api = { buildPlan, healthz, listPersonas };
