"""Rebrand this base repo for your organisation / vertical in one pass.

Rewrites the package name, the CLI entry point, the ``MKT_CAMPAIGN_`` env-var prefix, and the
...
(seed data, market / region, IdP wiring, eval golden set) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "campaign_planner"
_OLD_CLI = "mkt-campaign"
_OLD_ENV_PREFIX = "MKT_CAMPAIGN_"
_OLD_RESOURCE = "campaign-planner"
# In this repo the distribution name (pyproject ``name``) and the resource stem are the same
# literal string, so the dist default is the ``--resource`` value.
_OLD_DIST = "campaign-planner"
...
def _replacements(args: argparse.Namespace) -> list[tuple[str, str]]:
    ...
    # Order matters: the distribution name and the resource stem are the same literal here, so
    # the dist entry (first) rewrites every occurrence; the resource entry then matches none.
    # The CLI name is a prefix of the resource stem, so the longer, more specific strings must
    # come first to avoid a partial rewrite.
    return [
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``MKT_CAMPAIGN_`` when it prefixes an UPPER token
        # (MKT_CAMPAIGN_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-campaign-planner"
    )
    ap.add_argument(
        "--dist", default="", help="new distribution name (default: the --resource value)"
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (seed data, market/region, IdP).")
    return 0
