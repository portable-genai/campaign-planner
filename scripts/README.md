...
vertical, budget, pacing) scenarios spanning both verticals across JP, AU and SG. Run them
from the repo root with the domain package on the path:

```bash
export PYTHONPATH=src
export MKT_CAMPAIGN_PROFILE=local
```

| Script | What it does |
|--------|--------------|
| `demo.py` | Runs the real plan service over four scenarios, prints a cited trace to stdout, and writes each audit view to `scripts/out/*.json`. Also the end-to-end smoke test for the slice. |
| `render_plan_ui.py` | Dependency-free static HTML renderer: reads the `demo.py` JSON output and writes the audit-first console pages (`scripts/out/index.html` + one page per plan), the same palette and markup as the live Next.js console. |
| `demo_server.py` | Live, presenter-controlled HTTP server (stdlib only) that reveals one of the four plans per click, reusing `render_plan_ui.py` verbatim. `make demo-server`, then open `http://localhost:8111`. |
...
| `portability_demo.py` | Bounded offline profile and audit portability proof (see `../DEMO.md`), unrelated to the presenter walkthrough. |
