...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_plans.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden plans.", ""]
    return lines
...
def _exercised_block() -> list[str]:
    ...
    cases = load_jsonl(GOLDEN)
    benchmarks = run_eval.benchmarked_channels()
    priced = sum(len(channels) for channels in benchmarks.values())
    markets = sorted({row["market"] for row in cases})
    return [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden plans** in `eval/datasets/golden_plans.jsonl`, across",
        f"  {', '.join(markets)} and both verticals.",
        f"- **{priced} channel benchmarks from the SHIPPED demo book**, across",
        f"  {len(benchmarks)} market and vertical pairs. That is the oracle",
