"""Presenter-controlled Playwright walkthrough of the live D2 campaign-planner demo.

Drives a headed browser through the four-plan flow served by ``scripts/demo_server.py``.
...
Usage (two terminals)::
    ...
    make demo-server                                    # or: MKT_CAMPAIGN_PROFILE=local \\
...
Environment overrides:
    DEMO_URL    server base URL (default http://127.0.0.1:8111)
...
BASE = os.environ.get("DEMO_URL", "http://127.0.0.1:8111")
...
STEPS = [
    (
        "The console opens on a Singapore banking objective: 'savings account acquisition' "
        "at a 120,000 budget, evenly paced. The KPI tiles (budget, expected conversions, "
        "blended CAC, unique reach) are deterministic maths, and the HUMAN REVIEW REQUIRED "
        "banner gates the plan before any spend commits.",
        False,
        ".kpis",
    ),
    (
        "Next is a Japan banking launch: a multi-currency wallet at a 200,000 budget, "
        "front-loaded. The audience-segmentation panel scores each segment by propensity, "
        "with the consented-reachable count next to it: nothing unconsented gets targeted.",
        True,
        ".row",
    ),
    (
        "A Japan online-retail loyalty-programme push at 90,000, evenly paced: the channel "
        "mix panel is the deterministic budget-allocation engine, not the model, splitting "
        "spend across channels with the expected conversions per channel.",
        True,
        ".panel",
    ),
    (
        "An Australia online-retail seasonal sale at 110,000, back-loaded, closes the "
        "walkthrough: the flight schedule shows each leg's date range and share of spend. "
        "Every plan across all four scenarios cites its sources.",
        True,
        ".cites",
    ),
]
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        print("\n=== D2 campaign-planner live demo - press Enter to advance each step ===\n")
